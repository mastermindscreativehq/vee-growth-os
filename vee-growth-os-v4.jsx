...
// ╔══════════════════════════════════════════════════════════════════╗
// ║  VEE URBAN VOGUE — AI GROWTH OPERATING SYSTEM v3.5             ║
// ║  + Growth Memory · Strategy Tab · Decision Engine               ║
...
function StoreProvider({ children }) {
  ...
  // Persist to memory (simulating localStorage pattern for artifact compatibility)
  ...
  const actions = useMemo(() => ({
    ...
    toggleTask: (key) => dispatch({ type: "TOGGLE_TASK", payload: key }),
    addClient: (client) => dispatch({ type: "ADD_CLIENT", payload: client }),
    moveClient: (id, stage) => dispatch({ type: "MOVE_CLIENT", payload: { id, stage } }),
    removeClient: (id) => dispatch({ type: "REMOVE_CLIENT", payload: id }),
    addOpportunity: (opp) => dispatch({ type: "ADD_OPPORTUNITY", payload: opp }),
    removeOpportunity: (id) => dispatch({ type: "REMOVE_OPPORTUNITY", payload: id }),
    convertOpportunity: (id) => dispatch({ type: "CONVERT_OPPORTUNITY", payload: id }),
    addEvent: (evt) => dispatch({ type: "ADD_EVENT", payload: evt }),
    removeEvent: (id) => dispatch({ type: "REMOVE_EVENT", payload: id }),
    setOrders: (orders) => dispatch({ type: "SET_ORDERS", payload: orders }),
    setGrowthMetric: (m) => dispatch({ type: "SET_GROWTH_METRIC", payload: m }),
    setAiBriefing: (b) => dispatch({ type: "SET_AI_BRIEFING", payload: b }),
    setAiLoading: (l) => dispatch({ type: "SET_AI_LOADING", payload: l }),
    setContentCalendar: (c) => dispatch({ type: "SET_CONTENT_CALENDAR", payload: c }),
    setWeeklyReport: (r) => dispatch({ type: "SET_WEEKLY_REPORT", payload: r }),
    // v3.5 actions
    addMemory: (m) => dispatch({ type: "ADD_MEMORY", payload: m }),
    removeMemory: (id) => dispatch({ type: "REMOVE_MEMORY", payload: id }),
    updateMemory: (id, updates) => dispatch({ type: "UPDATE_MEMORY", payload: { id, updates } }),
    setTodayDecision: (d) => dispatch({ type: "SET_TODAY_DECISION", payload: d }),
    setWeeklyFocus: (f) => dispatch({ type: "SET_WEEKLY_FOCUS", payload: f }),
    // v4.0 actions
    addTimelineEvent: (e) => dispatch({ type: "ADD_TIMELINE_EVENT", payload: e }),
    removeTimelineEvent: (id) => dispatch({ type: "REMOVE_TIMELINE_EVENT", payload: id }),
  }), []);
...
// Need React reference for useReducer
